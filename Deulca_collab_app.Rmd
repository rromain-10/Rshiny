...
ui<- fluidPage(
  ...
  dashboardBody(
    sidebarLayout(
      sidebarPanel(
        checkboxGroupInput(inputId = "sample2", label = " Choose sample", selected=c("ARPE19", "T53D4", "RasV12","Aktmyr" ,"MekDD"),
                           c("ARPE19", "T53D4", "RasV12","Aktmyr" ,"MekDD")
                          )
        ),
      mainPanel(
...
server<- function(input,output, session) {
  ...
  ({
    ...
    output$whole_heatmap<- renderPlot ({
      print("rendering plot")
      # filtered4 = means_of_changing_genes2
      # melted4 = melt(filtered4)
      # colnames(melted4) <- c("gene","sample","counts")
      # print(input$sample2)
      # filtered4 = melted4[melted4$sample %in% input$sample2,]
      # filtered4
      # ggplot(melted4,
      #        aes(x= sample ,y= gene
      #            )) +
      ...
      # d3heatmap(counts_matrix,
      #           scale = 'row',
      #           colors= scales::col_quantile("RdYlBu",NULL,8),
      #           k_row =9,
      #           k_col = 5, 
      #           Rowv = T,
      #           Colv = T,
